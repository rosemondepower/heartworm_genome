...
bsub.py -q long --threads 4 20 coverage "./coverage.sh"
```

```bash
#!/bin/bash

WORKING_DIR=/lustre/scratch125/pam/teams/team333/rp24/DIRO/DATA/03_ANALYSIS/03_COVERAGE
cd ${WORKING_DIR}
...
for i in ../02_MAP/EXTRACT/*extract.bam; do
...
# do extra samples 
for i in ../02_MAP/EXTRACT/EXTRA/*extract.bam; do
...
for i in *.chr.cov; do
	name=${i%_extract.chr.cov};
	nuc=$(grep -v "scaffold\|Wb\|Mt\|NC\|NW" ${i%_extract.chr.cov}_extract.100000_window.cov | datamash mean 5 sstdev 5 );
...
setwd("C:/Users/rpow2134/OneDrive - The University of Sydney (Staff)/Documents/HW_WGS/R_analysis/batch4/coverage")
...
# list file names
file_names.window <- list.files(path = "C:/Users/rpow2134/OneDrive - The University of Sydney (Staff)/Documents/HW_WGS/R_analysis/batch4/coverage/input",pattern = "_extract.100000_window.cov")

# load data using file names, and make a formatted data frame
setwd("C:/Users/rpow2134/OneDrive - The University of Sydney (Staff)/Documents/HW_WGS/R_analysis/batch4/coverage/input")
...
data <- purrr::map_df(file_names.window, function(x) {
  ...
  cbind(sample_name = gsub("_extract.100000_window.cov","",x), data)
})
...
# lots of samples, can't fit into a single plot. Break up into 3 separate plots.

# part1
ggplot(subset(data_nuc, ID %in% unique(ID)[1:72]), aes(NUM, PROPORTION_COVERAGE/(median(PROPORTION_COVERAGE)), group = ID, col = CHR)) +
  geom_point(size=0.5) +
  labs( x = "Genome position" , y = "Relative coverage per 100kb window") +
...
# part2
ggplot(subset(data_nuc, ID %in% unique(ID)[73:144]), aes(NUM, PROPORTION_COVERAGE/(median(PROPORTION_COVERAGE)), group = ID, col = CHR)) +
  geom_point(size=0.5) +
  labs( x = "Genome position" , y = "Relative coverage per 100kb window") +
  theme_bw() + theme(strip.text.x = element_text(size = 8),
                     axis.text.x = element_text(size = 10),
...
# part1
ggplot(subset(data_nuc, ID %in% unique(ID)[1:72]), aes(NUM, PROPORTION_COVERAGE, group = ID, col = CHR)) +
  geom_point(size=0.5) +
  labs( x = "Genome position" , y = "Coverage per 100kb window") +
  theme_bw() + theme(strip.text.x = element_text(size = 8),
                     axis.text.x = element_text(size = 10),
                     axis.text.y = element_text(size = 10),
                     axis.title.x = element_text(size = 20),
...
ggsave("../ALL_genomewide_RAWcoverage_allsamples_part1.png", height=16, width=12)
ggsave("../ALL_genomewide_RAWcoverage_allsamples_part1.tif", height=16, width=12, dpi = 300)

# part2
ggplot(subset(data_nuc, ID %in% unique(ID)[73:144]), aes(NUM, PROPORTION_COVERAGE, group = ID, col = CHR)) +
  geom_point(size=0.5) +
  labs( x = "Genome position" , y = "Coverage per 100kb window") +
  theme_bw() + theme(strip.text.x = element_text(size = 8),
                     axis.text.x = element_text(size = 10),
...
ggsave("../ALL_genomewide_RAWcoverage_allsamples_part2.png", height=16, width=12)
ggsave("../ALL_genomewide_RAWcoverage_allsamples_part2.tif", height=16, width=12, dpi = 300)
...
# part2
data_nuc %>%
  filter(., grepl("chrX|chr1",CHR) & ID %in% unique(ID)[73:143]) %>%
  ggplot(aes(NUM, PROPORTION_COVERAGE/(median(PROPORTION_COVERAGE)), group = ID, col = CHR)) +
  geom_point(size=0.2) +
  labs( x = "Genome position" , y = "Relative coverage per 100kb window") +
  theme_bw() + theme(strip.text.x = element_text(size = 8),
